...
<!--
    This xs3p.xsl file was copied from Karim Ratib's version of xs3p project,
    it is available at https://github.com/infojunkie/xs3p

    It has been modified by NoteLite Contributors for NoteLite documentation:
    - Documentation popups can be replaced by inline sections.
        See 'xmlDocWidth' param
    - Intermediate elements can be avoided to allow direct navigation to relevant type.
        See 'useTypeShortcut' param
    - Documentation text can use HTML tags (as in JavaDoc) rather than Markdown
        See 'useMarkdown' param
        
    The default values for these 3 new parameters are such that, by default,
    no modification appears in the resulting view.
-->
<!--
...
<!--     <xsl:character-map name="esc-map">
    ...
    <!--<xsl:param name="bootstrapURL">https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/5.1.3</xsl:param>-->
    ...
    <!-- Style width for doc, according to xmlDocWidth parameter -->
    ...
    <xsl:template match="/xsd:schema">
        ...
        <!-- HB: Confirm user choices (to be removed) -->
        <xsl:message>
        ...
        <xsl:message>
            <xsl:text>useTypeShortcut= </xsl:text>
            <xsl:value-of select="$useTypeShortcut"/>
        </xsl:message>
        <xsl:message>
            <xsl:text>useMarkdown= </xsl:text>
            <xsl:value-of select="$useMarkdown"/>
        </xsl:message>
        <xsl:message>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-9 content" role="main" id="xs3p-content">
                            ...
                            <section id="SectionSchemaProperties">
                                <h2>
                                    <a id="SchemaProperties">Schema Document Properties</a>
                                </h2>
                            ...
                            <xsl:if test="xsd:redefine">
                                <h2>
                                    <a id="Redefinitions">Redefined Schema Components</a>
                                </h2>
                            ...
                            <xsl:choose>
                                ...
                                <xsl:when test="normalize-space(translate($sortByComponent,'TRUE','true'))='true'">
                                    ...
                                    <xsl:if test="xsd:attribute or xsd:element">
                                        <section id="SectionSchemaDeclarations">
                                            <h2>
                                                <a id="SchemaDeclarations">Global Declarations</a>
                                            </h2>
                                    ...
                                    <xsl:if test="xsd:attributeGroup or xsd:complexType or xsd:group or xsd:notation or xsd:simpleType">
                                        <section id="SectionSchemaDefinitions">
                                            <h2>
                                                <a id="SchemaDefinitions">Global Definitions</a>
                                            </h2>
                                ...
                                <xsl:otherwise>
                                    <h2>
                                        <a id="SchemaComponents">Global Schema Components</a>
                                    </h2>
    ...
    <xsl:template match="xsd:*[@name]" mode="toc">
        <!-- HB: Exclude the nodes with type attribute? -->
        <xsl:if test="not(@type) or normalize-space(translate($useTypeShortcut,'TRUE','true'))='false'">
...
.xs3p-xml-line {            <!-- HB: Container for an XML line -->
    display: flex;
    ...
    <xsl:template name="PrintAnnotation">
        ...
        <xsl:if test="$component/xsd:annotation/xsd:documentation">
            <xsl:for-each select="$component/xsd:annotation/xsd:documentation">
                <xsl:choose>
                    <xsl:when test="$useMarkdown='true'">
                        <xsl:if test="position()!=1">
                            <br/>
                            <br/>
                        </xsl:if>
        ...
        <xsl:if test="normalize-space(@maxOccurs)!='0'">
            <div class="xs3p-xml-line">
                <div class="xs3p-xml-part">
                    <xsl:if test="$debug='true'"><xsl:text>BINGO-A </xsl:text></xsl:if>
            ...
            <div class="xs3p-xml-line">
                <div class="xs3p-xml-part">
                    <xsl:if test="$debug='true'"><xsl:text>BINGO-B </xsl:text></xsl:if>
                    <span class="group" style="margin-left: {$margin}em">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <div class="xs3p-xml-line">
                    ...
                    <!-- Documentation (inline) -->
                    <xsl:if test="local-name(..)!='schema' and $xmlDocWidth!=0">
                        <!--<xsl:apply-templates select="." mode="hiddendoc"/>-->
                        <xsl:call-template name="PrintSampleDocumentation">
                            <xsl:with-param name="component" select="."/>
                        </xsl:call-template>
...
<!--                <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="normalize-space($defLoc)='' or normalize-space($defLoc)='none' or normalize-space($defLoc)='xml' or normalize-space($defLoc)='xsd'">
                        <div class="xs3p-xml-line">
                            <div class="xs3p-xml-part">
                                <xsl:if test="$debug='true'"><xsl:text>BINGO-E </xsl:text></xsl:if>
            ...
            <xsl:otherwise>
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="normalize-space($grpDefLoc)='' or normalize-space($grpDefLoc)='none' or normalize-space($grpDefLoc)='xml' or normalize-space($grpDefLoc)='xsd'">
                        <div class="xs3p-xml-line">
                            <div class="xs3p-xml-part">
                                <xsl:if test="$debug='true'"><xsl:text>BINGO-I </xsl:text></xsl:if>
                                <div class="other" style="margin-left: {$margin}em">
    ...
    <!--
       Prints out a schema component's documentation.
         - If global param xmlDocWidth is 0, a popup window is used to display documentation
         - Else documentation is printed into its reserved room
       Param(s):
              component (Node) required
                  Schema component
    -->
    <xsl:template name="PrintSampleDocumentation">
        ...
        <xsl:if test="$component and $component/xsd:annotation/xsd:documentation">
            <xsl:choose>
                <xsl:when test="$xmlDocWidth=0">
                    <!-- Use of popup for doc -->
                    <xsl:variable name="documentation">
                        <xsl:for-each select="$component/xsd:annotation/xsd:documentation">
                            <xsl:if test="position()!=1">
                                <xsl:text>,</xsl:text>
    ...
    <xsl:template name="PrintSampleGroup">
        ...
        <xsl:if test="normalize-space($occursInfo)!='[1]'">
            ...
            <div class="xs3p-xml-line">
                <div class="xs3p-xml-part">
                    <xsl:if test="$debug='true'"><xsl:text>BINGO-L </xsl:text></xsl:if>
    ...
    <xsl:template name="PrintSampleSimpleElement">
        ...
        <!-- HB Detect if we can apply a type shortcut
         If input like : <xs:element ref="tag"/>
         Rather than   : <tag>...</tag>  w/ hlink on tag
         Use directly  : <tag>type</tag> w/ hlink on type
        -->
        <xsl:variable name="shortcut">
            <xsl:if test="$useTypeShortcut='true' and $element/@ref and not($element/@name)">
                <xsl:variable name="elem2" select="key('element', $element/@ref)"/>
                <xsl:if test="$elem2">
                    <xsl:value-of select="$elem2/@type"/>
                </xsl:if>
            </xsl:if>
        </xsl:variable>
        ...
        <xsl:variable name="elemTag">
            ...
            <xsl:choose>
                ...
                <xsl:when test="$element/@ref">
                    <xsl:choose>
                        <!-- HB Just the tag name without hlink -->
...
<!--        <xsl:choose>
        ...
        <div class="xs3p-xml-line">
            ...
            <xsl:if test="local-name($element/..)!='schema' and $xmlDocWidth!=0">
                <!-- Documentation (no popup) -->
                <xsl:call-template name="PrintSampleDocumentation">
                    <xsl:with-param name="component" select="$element"/>
                </xsl:call-template>
            </xsl:if>
...
<!--                <xsl:choose>
                ...
                <div class="xs3p-xml-line">
                    <div class="xs3p-xml-part">
                        <xsl:if test="$debug='true'"><xsl:text>BINGO-O </xsl:text></xsl:if>
...
<!--                        <xsl:message><xsl:text>XXX</xsl:text><xsl:value-of select="$tag"/></xsl:message>-->
...
<!--        </div>-->
    ...
    <xsl:template name="DisplaySchemaComponent">
        ...
        <!--<div class="xs3p-xml-line">  Safer -->
        <xsl:if test="$debug='true'"><xsl:text>BINGO-U </xsl:text></xsl:if>
