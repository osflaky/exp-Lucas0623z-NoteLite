...
NoteLite can export a transcribed score into the *de facto* standard MusicXML format for an easy
exchange with other music applications.
But this MusicXML data is only a part of the OMR data produced by NoteLite transcription process.

NoteLite stores all its transcription data about a given Book into a dedicated project file,
...
As explained in
[HandBook .omr files](https://notelite.example.com/notelite/_pages/outputs/omr/) section,
...
[NoteLite Releases](https://github.com/ORG/notelite/releases) section
on GitHub site.
Typical archive name is `NoteLite_Schemas_Doc-X.Y.Z.zip`, where X.Y.Z is the release version.
...
Now that `xsltproc` is up and running, here are the steps to perform on NoteLite project:

1. If not yet done, clone NoteLite project from GitHub to your local machine
```bash
 git clone https://github.com/ORG/notelite.git notelite
```
2. Move to the `notelite` directory just created
```bash
 cd notelite
```
...
total 900
-rwxrwxr-x+ 1 919398 Jan 24 20:18 NoteLite_Schemas_Doc-5.3-alpha.zip
```
