...
import static com.notelite.omr.util.UriUtil.toURI;
...
public abstract class WellKnowns
{
    ...
    /** Flatpak id (com.notelite.notelite or null). */
    ...
    private static Path getFolderForLinux (FolderKind kind)
    {
        ...
        if (xdg != null) {
            ...
            final Path notelitePath = Paths.get(FLATPAK ? xdg : xdg + TOOL_PREFIX);

            return switch (kind) {
                case DATA -> notelitePath;
                case CONFIG -> notelitePath;
                case LOG -> notelitePath.resolve("log");
            };
        }
    ...
    private static Path getFolderForWindows (FolderKind kind)
    {
        ...
        final Path notelitePath = Paths.get(appdata + TOOL_PREFIX);
        ...
        return switch (kind) {
            ...
            case CONFIG -> notelitePath.resolve("config");
            case LOG -> notelitePath.resolve("log");
        };
    }
    ...
    private static JarFile getJarFile ()
    {
        try {
            ...
            if (en.hasMoreElements()) {
                ...
                // url = jar:http://notelite.kenai.com/jnlp/notelite.jar!/omr/WellKnowns.class
